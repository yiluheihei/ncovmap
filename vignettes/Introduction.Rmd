...
```{r map_hubei}
devtools::install_github("yiluheihei/ncovmap")
library(ncovmap)
# 例数为0的城市以白色突出显示
...
```{r all-province}
# 没有台湾,香港,澳门等三地数据
provinces <- leafletCN::regionNames("china") %>% 
  setdiff(c("台湾省", "香港特别行政区", "澳门特别行政区"))


htmltools::tagList(purrr::map(provinces, 
  ~ plot_province_map(ncov, .x, bins = c(0, 10, 100, 500, 1000, 10000))))
```
...
```{r shanghai-bin}
plot_province_map(ncov, "上海市", bins = c(0, 5, 10, 20, 60))
```
